...
class MemoryTracker:
    ...
    def start(self):
        if self.gpu:
            self.gpu_memory_start = torch.cuda.memory_stats()['allocated_bytes.all.allocated']
        self.cpu_memory_start = psutil.virtual_memory().used

    def end(self):
        if self.gpu:
            self.gpu_memory_end = torch.cuda.memory_stats()['allocated_bytes.all.allocated']
            torch.cuda.synchronize()
            self.gpu_memory_values = torch.cat(
                (self.gpu_memory_values, torch.tensor([[self.gpu_memory_end - self.gpu_memory_start]]))
            )
        self.cpu_memory_end = psutil.virtual_memory().used
        self.cpu_memory_values = torch.cat(
            (self.cpu_memory_values, torch.tensor([[self.cpu_memory_end - self.cpu_memory_start]]))
        )
...
class MemoryTrackers:
    ...
    def evaluate_run(self):
        ...
        box_values = torch.empty(size=(0, len(self.memory_trackers[item].cpu_memory_values.squeeze())))
        labels = []
        for item in self.memory_trackers.keys():
            if len(self.memory_trackers[item].cpu_memory_values) == 0:
                continue
        ...
        fig, axs = plt.subplots(figsize=(15, 10))
        box_values = torch.empty(size=(0, len(self.memory_trackers[item].gpu_memory_values.squeeze())))
        labels = []
        for item in self.memory_trackers.keys():
            if len(self.memory_trackers[item].gpu_memory_values) == 0:
                continue
